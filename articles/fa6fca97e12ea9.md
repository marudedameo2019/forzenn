...
今回は画面ロード時にCSVをパースしているので、うちのPCで200msほど、うちのスマホだと2～3秒ほどかかりました。19MB程度あるので、通信なのか解析なのかそれなりに時間がかかるようです。実用上はCSVでないデータ形式からロードする形にすべきでしょうね。
...
特性上はとにかく低コスト、低機能という感じです。Trie木構築の時間もスマホで200ms程度なので、低コストです。もちろんCSV読み込み時間(2～3秒)は別途かかります。
...
|CSV読み込み時間|CSVをダウンロードしてPapaParseでパースし行データの配列として格納するまでの時間です|
...
- PapaParse: CSVパース用に使用。ブラウザ/node.jsで使える古くメジャーなCSVパーサー
