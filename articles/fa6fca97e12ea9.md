...
今回は画面ロード時にCSVをパースしているので、うちのPCで~~200~~400msほど、うちのスマホだと~~2～3秒~~500ms(csv-pipeに変えて通信が圧縮されるようになった)ほどかかりました。19MB程度あるので、通信なのか解析なのかそれなりに時間がかかるようです。実用上はCSVでないデータ形式からロードする形にすべきでしょうね。
