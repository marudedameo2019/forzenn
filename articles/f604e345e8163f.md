...
まずはデータベーススキーマから。Laravelではまずテーブルありきでモデルが決まるようなので。
...
前述したようにデータベーススキーマを元に生成されるようなので、モデル自体を事細かに記述する必要はないのですが、バルク処理(`fillable`)とリレーション用の記述は必要なようです。

### ファクトリ

次に作ったモデルのダミーデータをどう作るかを実装します。テストとかで使います。
...
ダミーデータを作る際に使用するコードです。何個作るかみたいなところを制御します。
